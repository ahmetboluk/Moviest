...
public class TabOne extends Fragment implements BottomTabLayotListener {
        ...
        recyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (totalItemCount > 0 && endHasBeenReached) {
                    ...
                    });}else if(SELECTED==SELECTED_TV){api.create(TmdbApi.class).listSeriesPopular(API_KEY,page).enqueue(new Callback<SeriesPageData>() {
                        @Override
    ...
    public void onSeriesSelected() {
        //Log.d("BottomSelected","SeriesSelected TabOne");
        SELECTED=SELECTED_TV;
        if(seriesResultList.isEmpty()){
            page=1;
        api.create(TmdbApi.class).listSeriesPopular(API_KEY,page).enqueue(new Callback<SeriesPageData>() {
            @Override
            public void onResponse(Call<SeriesPageData> call, Response<SeriesPageData> response) {
